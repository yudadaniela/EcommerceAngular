...
import { Region, SmallCountry, Currencies, Name } from '../../../Models/country-interface';
import { filter, switchMap, tap } from "rxjs";
import { AuthService } from "../../../services/auth.service";
import { MatSnackBar } from '@angular/material/snack-bar';
import { createPasswordStrengthValidator, emailPattern, passwordMatchValidator } from "../validators/validator";
...
export class SignUpComponent implements OnInit{
  ...
  onSubmit( email:string, password:string){
    ...
    this.authService.createUser(emailAndPassword.email, emailAndPassword.password)
    . then(response=>{
  ...
  onRegionChanged():void{
    this.formRegister.get('region')!.valueChanges
    .pipe(
  ...
  onCountryChange():void{
    this.formRegister.get('country')!.valueChanges
    .pipe(
